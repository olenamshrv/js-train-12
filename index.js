...
function createDictionary(entries) {
  ...
  // entries.forEach((entry) => dictionary.set(entry[0], entry[1]));
...
function sortByValues(dictionary) {
  ...
  let arr = [];
  dictionary.forEach((value, key) => arr.push([key, value]));

  let sortedArr = arr.sort((elem1, elem2) => {
    return elem1[1] < elem2[1] ? 1 : -1;
  });

  let map = new Map();
  sortedArr.forEach(([key, value]) => map.set(key, value));
  return map;
}
